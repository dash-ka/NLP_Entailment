def train(model,
    ...
    for batch_dictionary in generate_batches(train_data, voc, device = device):  
        ...
        # compute output 
        log_probs, _ = model(prem, hypo, len_prem)
...
def predict(premise, model, device, max_len=20):
    ...
    with torch.no_grad():
        ...
        for step in range(max_len):
            ...
            model.decoder(dec_input, dec_state, enc_out, enc_state, mask_prem, mask_prem)
            attns.append(attn)
